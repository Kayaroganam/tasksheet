...
def newtest_task(request):
    task_sheet = newtest.objects.all().values()
    template = loader.get_template('show_Task_Sheet.html')
    context = {
        'task_sheet' : task_sheet
    }
...
def newtest_add_task(request):
    return render(request, 'add.html')

def newtest_adding(request):
    a = request.POST['serial_no']
    b = request.POST['project_no']
    c = request.POST['project_name']
    d = request.POST['process_id']
    ...
    i = request.POST['date']
    j = request.POST['hours']
    task = newtest(s_no=a, project_no=b, project_name=c, process_id=d, process_sequence=e, estimated_hours=f, tasks=g, server_link=h, date=i, hours=j)
    task.save()
    return HttpResponseRedirect(reverse('newtest_task'))

def newtest_delete(request, id):
    task = newtest.objects.get(id=id)
    task.delete()
    return HttpResponseRedirect(reverse('newtest_task'))

def newtest_edit(request, id):
    task = newtest.get_template("edit.html")
    context = {
        'task':task
    }
    return HttpResponse(template.render(context, request))

def newtest_editrecord(request, id):
    task = newtest.objects.get(id=id)
    a = request.POST['serial_no']
    b = request.POST['project_no']
    c = request.POST['project_name']
